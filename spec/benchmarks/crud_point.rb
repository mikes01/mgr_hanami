...
class CrudPointTests
  def initialize(repeat_times)
    ...
    @update_time = 0
    @delete_time = 0
    @file_name = "./hanami_crud_point_model_#{@repeat_times}.csv"
  ...
  def add
    @entity = @repository.create({name: 'test',
      coordinates: @p1,
  ...
  def read
    @repository.find(@entity.id)
  end

  def update
    @repository.update(@entity.id, {name: 'updated',
